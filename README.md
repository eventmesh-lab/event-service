# Plantilla event_service Hexagonal (dotnet)
...
- src/
  - event_service.Domain/
  - event_service.Application/
  - event_service.Infrastructure/
  - event_service.Api/
- tests/
  - event_service.Domain.Tests/
  - event_service.Application.Tests/
  - event_service.Infrastructure.IntegrationTests/
...
```bash
git clone https://github.com/<owner>/event_service-hexagonal-template-.git
cd event_service-hexagonal-template-
...
dotnet new event_service-hex -n Orders -o ./Orders --framework net8.0
...
Notas importantes
- El template usa `event_service` como `sourceName`; al generar el proyecto ese token se sustituye por el nombre que pases con `-n`.
...
dotnet new uninstall /path/to/event_service-hexagonal-template-
...
- Si el comando `dotnet new event_service-hex` no aparece tras instalar, ejecuta `dotnet new install . --force` y verifica con `dotnet new list`.
